...
import { PALETTE, deal } from './palette.js';
...
/* Every manta is bright and saturated now: v1.8 retires the dark wild manta
   along with the conditions that needed it. One base level for everyone, and
   your train burns hotter on top of it. */
...
export function createMantas (scene) {
  ...
  /* A saturated hue at full value cannot get brighter in its own channels, so
     your train's extra comes out of the saturation too. */
  const yoursTint = hex => {
    const t = tint(hex, BASE_LEVEL * YOURS_GAIN);
    const peak = Math.max(t[0], t[1], t[2]);
    return t.map(v => v + (peak - v) * YOURS_WHITE);
  };
  ...
  function rollColours (newSeed) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      else if (r.kind === 'rival') t = tint(dealt.rivals[r.rival].hex, BASE_LEVEL);
      else t = tint(dealt.wilds[r.idx].hex, BASE_LEVEL);
      r.tint = t;
      baseTint[i] = t.slice();
      applyTint(i);
    }
  ...
  function update (secs) {
    ...
    for (let r = 0; r < RIVAL_TRAINS; r++) {
      ...
      const headS = trail.length ? trail[trail.length - 1].s : 0;
      for (let k = 1; k < RIVAL_LEN; k++) {
        const want = headS - k * SPACING;
        let pt = trail[0];
        for (let q = trail.length - 1; q >= 0; q--) { if (trail[q].s <= want) { pt = trail[q]; break; } }
  ...
  return { mesh, update, setBounds, count: COUNT, aPos, aHead, aSize, aTint, aMotion, vertexBuffers, isWild,
           ...
           gainFor: key => {
             ...
             return L(yoursTint(c.hex)) / L(tint(c.hex, BASE_LEVEL));
           },
