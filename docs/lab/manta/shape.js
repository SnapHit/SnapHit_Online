...
/* One strip per wing across the stations, so every vertex at the same
   ...
   along its length instead of hinging at a hub. Returns the geometry plus two
   per-vertex attributes: how bright this part of the animal is, and how far
   along the tail it sits. */
export function mantaGeometry () {
  const pos = [], shade = [], tail = [];
  const push = (x, z, sh, tl) => { pos.push(x, 0, z); shade.push(sh); tail.push(tl); };
  ...
  for (const sign of [1, -1]) {
    for (let i = 0; i < A.length - 1; i++) {
      ...
      push(sign * a0, L0, 1, 0); push(sign * a0, T0, 1, 0); push(sign * a1, L1, 1, 0);
      push(sign * a0, T0, 1, 0); push(sign * a1, T1, 1, 0); push(sign * a1, L1, 1, 0);
    }
  }

  /* The tail: dark where it leaves the body, pale for its rear two thirds. */
  const tz = k => HEAD_FRONT + BODY_BACK + TAIL_LEN * k;
  const tw = k => lerp(TAIL_W0, TAIL_W1, k);
  const tshade = k => (k < 1 / 3 ? lerp(0.50, 0.70, k * 3) : lerp(0.95, 1.10, (k - 1 / 3) * 1.5));
  for (let i = 0; i < TAIL_SEGS; i++) {
    const k0 = i / TAIL_SEGS, k1 = (i + 1) / TAIL_SEGS;
    const z0 = tz(k0), z1 = tz(k1), w0 = tw(k0), w1 = tw(k1);
    const s0 = tshade(k0), s1 = tshade(k1);
    push(-w0, z0, s0, 1 + k0); push( w0, z0, s0, 1 + k0); push(-w1, z1, s1, 1 + k1);
    push( w0, z0, s0, 1 + k0); push( w1, z1, s1, 1 + k1); push(-w1, z1, s1, 1 + k1);
  }

  const g = new BufferGeometry();
  g.setAttribute('position', new Float32BufferAttribute(pos, 3));
  g.setAttribute('aShade',   new Float32BufferAttribute(shade, 1));
  g.setAttribute('aTail',    new Float32BufferAttribute(tail, 1));
  return g;
}
