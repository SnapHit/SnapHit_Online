...
- Wild mantas spread evenly across the ocean in small groups on a lattice, drawn gently towards plankton blooms and regrowing away from leaders towards their count. Regrowth refills the gap: the more mantas are missing, the faster they return, so the ocean settles near its count however hungry the trains are. A fixed trickle of one every 0.5 seconds drained an ocean of 300 to about 35 within two minutes of ten bots eating. Their layout (group homes, blooms and spawn places) comes from the current arena radius and wild count, and is rebuilt when either changes; a layout kept from a larger arena once left 85 to 91 percent of wild mantas pinned against the reef. Evenness is judged by where the groups are, not by each manta, since groups of 3 to 5 are the design. Crash and cut debris is separate from that count: it glows and can be collected, then sinks away, so crashes can neither empty nor flood the ocean. Under a crash every half second for a minute, the wild count held at 19 to 20 of 20, where counting debris against it had collapsed it to about 4.
...
| Wild mantas | 300 (P) | In groups of 3 to 5 on an even lattice, away from leaders. Each missing manta returns on average within the refill time, 5 seconds (P), so regrowth runs faster the emptier the ocean. The gap settles at the eating rate times the refill time: eleven trains eat about 6 mantas a second, so 5 seconds holds about 86 percent of 300, 20 seconds about 58 percent and 40 seconds about 37 percent. Crash debris is counted separately and never stops regrowth. Until September 2026 every lab game started with 120 by mistake, whatever the setting, so earlier tuning of this value is void; Nathan's retune, with the smaller arena, makes the ocean roughly 60 times denser than 20 in radius 4,000 |
...
- Each phone is sent only what is near its view (interest management), plus the leaderboard. A slow mobile link measured 0.4 Mbit/s, about 50 KB a second, so a phone's stream stays well under that; sending all 300 wild mantas' positions 20 times a second would not fit.
...
- Rival leaders are drawn a fraction of a second in the past, smoothly between snapshots (interpolation). The delay starts at about 100 ms (P) and grows with the connection's jitter: on 4G the gap between replies varied by about 100 ms.
- A small signal mark shows the connection's quality, so a player on a poor link knows why a cut felt off.
- Crashes and cuts happen when the server says, and the set pieces play from its events.
- Cuts are judged where the server has the trains, with lag compensation: the server judges your leader against rival trains where your phone showed them when you steered. Measured on the Pixel 9 over 4G in September 2026, a round trip to Cloudflare's edge took 318 ms at the median and 377 ms at the 95th percentile. At a burst speed of 470 that is about 150 units of disagreement, several follower gaps, so without it a cut that looked clean on the phone would miss. On wifi the same round trip took 17.5 ms, so Cloudflare's part is small and the rest is the mobile link. The server rewinds by at most 250 ms (P): beyond that, a player on a very slow link could cut a train everyone else saw had already passed.
...
- **Snowballing.** Only leaders recruit, big trains are big targets, plankton blooms and events redistribute mantas, and a train can only grow as fast as the ocean refills. A fuller ocean trades against this: bot matches measure how often one train holds more than half of all the mantas in the water. The first greybox build held 300 wild mantas by respawning each one instantly, and trains grew to ridiculous lengths within minutes.
